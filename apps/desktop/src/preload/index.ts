...
const api = {
  ...
  /**
   * Clawdbot Integration API (OPT-IN)
   * Autonomous AI worker in Docker container
   */
  clawdbot: {
    ...
    getConfig: (): Promise<{
      ...
      gatewayToken?: string;
    }> => ipcRenderer.invoke('clawdbot:getConfig'),

    /** Configure (enable/disable, URLs) */
    configure: (config: {
      enabled?: boolean;
      url?: string;
      wsUrl?: string;
      ...
      ipcRenderer.invoke('clawdbot:configure', config),

    /** Get status (enabled, docker available, container running) */
    status: (): Promise<{
      ok: boolean;
      enabled: boolean;
    ...
    /** Health check (reach gateway) */
    health: (): Promise<{ ok: boolean; error?: string }> =>
      ipcRenderer.invoke('clawdbot:health'),

    /** Execute a task prompt */
    task: (prompt: string): Promise<{ ok: boolean; result?: string; error?: string }> =>
      ipcRenderer.invoke('clawdbot:task', prompt),

    /** Get provider configuration from Docker volume */
    getProviderConfig: (): Promise<{
      ok: boolean;
      config?: {
        lmstudio?: { enabled: boolean; baseUrl: string; models: Array<{ id: string; name: string; contextWindow?: number; maxTokens?: number }>; primaryModel?: string };
        ollama?: { enabled: boolean; baseUrl: string; models: Array<{ id: string; name: string }>; primaryModel?: string };
        anthropic?: { enabled: boolean; apiKey: string };
        openai?: { enabled: boolean; apiKey: string };
      };
      error?: string;
    }> => ipcRenderer.invoke('clawdbot:getProviderConfig'),
    ...
    setProviderConfig: (config: {
      ...
      ipcRenderer.invoke('clawdbot:setProviderConfig', config),

    /** Test provider connection */
    testProvider: (provider: 'lmstudio' | 'ollama' | 'anthropic' | 'openai', config: { baseUrl?: string; apiKey?: string }): Promise<{
      ok: boolean;
      error?: string;
      models?: Array<{ id: string; name: string; contextWindow?: number; maxTokens?: number }>;
    }> => ipcRenderer.invoke('clawdbot:testProvider', provider, config),

    /** Get recent task results */
    getRecentTasks: (limit?: number): Promise<{ ok: boolean; tasks?: unknown[]; error?: string }> =>
      ipcRenderer.invoke('clawdbot:getRecentTasks', limit || 5),
  },
  /**
