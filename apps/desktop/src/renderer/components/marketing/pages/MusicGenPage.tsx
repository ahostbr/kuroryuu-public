...
export function MusicGenPage() {
  ...
  const filteredJobs = useMarketingStore((s) => s.activeJobs).filter((j) => j.type === 'music');
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-zinc-700 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-zinc-100">Music Generation</h2>
            <p className="text-xs text-zinc-400 mt-0.5">Create custom background music and soundtracks</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div className="bg-zinc-800 rounded-lg border border-zinc-700 p-5 space-y-4">
          ...
          <button
            ...
            disabled={musicLoading || !prompt.trim()}
