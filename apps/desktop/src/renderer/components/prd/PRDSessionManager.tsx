/**
 * PRD Session Manager Flyout
 *
 * Allows saving, loading, and managing PRD sessions
 * Follows the pattern from Ideation SessionManager
 */
...
import type { PRDSession } from '../../types/prd';
...
function SaveDialog({ onSave, onCancel, prdCount }: SaveDialogProps) {
      ...
      <div className="space-y-3">
        <div>
          ...
          <input
            ...
            placeholder="e.g., Q1 Feature PRDs"
            className="w-full px-3 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:border-blue-500/50"
            autoFocus
          />
        </div>
        ...
        <div className="text-xs text-muted-foreground">
          Saving {prdCount} PRD{prdCount !== 1 ? 's' : ''}
        </div>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed transition-colors"
          >
            {isSaving ? (
...
// ============================================================================
// PRDSessionManager - Main Flyout
// ============================================================================
export function PRDSessionManager() {
  const {
    prds,
    sessions,
    currentSessionId,
  ...
  const handleExport = async (sessionId: string, format: 'markdown' | 'json') => {
    ...
    if (content) {
      // Copy to clipboard (toast already shown by exportSession)
        ...
        <div className="px-4 py-3 border-b border-border space-y-2">
          {activePRDs.length > 0 ? (
            <>
              {showSaveDialog ? (
                <SaveDialog
                  ...
                  prdCount={activePRDs.length}
                />
              ) : (
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowSaveDialog(true)}
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition-colors text-sm"
                  >
                    <Save className="w-4 h-4" />
                    Save Current ({activePRDs.length})
                  </button>
                  <button
            ...
            <div className="text-center py-4">
              ...
              <p className="text-sm text-muted-foreground">No PRDs to save</p>
              <p className="text-xs text-muted-foreground mt-1">Generate PRDs first</p>
            </div>
          )}
        </div>
          ...
          {isLoadingSessions ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Save your PRDs to access them later
              </p>
            </div>
          )}
        </div>
