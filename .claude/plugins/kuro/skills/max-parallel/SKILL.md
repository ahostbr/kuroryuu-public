...
For autonomous coding tasks that need Desktop visibility, use CLI mode to spawn agents via `k_bash`.

### When to Use CLI Mode

| Use Case | Mode |
|----------|------|
| Quick exploration, analysis | Native (Task tool) |
| Autonomous coding tasks | **CLI mode** |
| Need Desktop monitoring | **CLI mode** |
| Long-running implementations | **CLI mode** |

### CLI Mode Syntax

```bash
# Spawn CLI agents (Desktop visible)
/max-parallel --cli "implement feature X"

# With full autonomy (no permission prompts)
/max-parallel --cli --unattended "implement feature X"

# Show plan without executing
/max-parallel --dry-run "implement feature X"
```

### CLI Spawn Command Templates

**Default (restricted tools):**
```
k_bash(
  command='claude -p "Your task prompt" --allowedTools "Read,Glob,Grep,Write,Edit"',
  background=true,
  pty=true,
  wave_id="wave1"
)
...
```
k_bash(
  command='claude -p "Your task prompt" --dangerously-skip-permissions --allowedTools "Read,Glob,Grep,Write,Edit,Bash"',
  background=true,
  pty=true,
  wave_id="wave1"
)
...
## Permission Flag Decision Matrix

| Scenario | `--dangerously-skip-permissions` | `--allowedTools` |
|----------|----------------------------------|------------------|
...
| CI/CD pipelines | **Yes** | Defense in depth |
...
- For full autonomy, combine both: `--dangerously-skip-permissions --allowedTools "..."`

---

## Desktop Monitoring

When using `--cli` mode, agents appear in:
**Desktop → Coding Agents → Agent Flow**

### Wave Visualization

Sessions are grouped by `wave_id` metadata:
- Wave 1 agents cluster together
- Wave 2 agents show dependency arrows from Wave 1
- Real-time streaming via WebSocket

### Session Actions

From Agent Flow panel:
- **VIEW** - Open session log viewer
- **KILL** - Terminate agent process
- Click node to see full output

---

## Comparison: /max-parallel vs /max-swarm

| Feature | /max-parallel | /max-swarm |
|---------|---------------|------------|
| Dependency tracking | **Yes (waves)** | No |
| Desktop integration | With --cli flag | Always |
| Use case | Structured multi-step | Quick parallel burst |
| Execution modes | Native + CLI | CLI only |

Use `/max-parallel` for complex tasks with dependencies.
Use `/max-swarm` for quick parallel bursts without dependency management.
