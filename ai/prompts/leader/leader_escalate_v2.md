# Leader Escalation Ladder V2 (Superpowers-Enhanced)

> **V2 Changes:** Added leader self-check red flags, escalation rationalization prevention, and Level 4 evidence gate from Superpowers discipline techniques. Original: leader_escalate.md
...
## [V2] Leader Self-Check — Red Flags Before Escalation

BEFORE escalating to ANY level, ask yourself:

**Am I escalating based on evidence or emotion?**
- Impatience is not evidence. Verify the worker is actually stuck.
- Silence is not failure. Workers may be in deep execution.
- Slow progress is not no progress. Check heartbeat and recent updates.

**Red Flags — STOP and reassess:**
- Escalating because YOU are anxious, not because worker is stuck
- Jumping levels (0→3) without evidence at each intermediate level
- Sending hints without first reading PTY output (Level 1)
- Assuming worker is wrong without verifying their claims (Level 2)
- Sending multiple nudges without waiting for response
- Hotfixing (Level 4) because it's "faster" when Level 3 hasn't been tried
- Escalating on a task worker hasn't even claimed yet

**The rule:** Evidence first, action second. Every escalation level requires evidence from the previous level.

---

## [V2] Escalation Rationalizations

| Excuse | Reality |
|--------|---------|
| "Worker has been silent for a while" | Define "a while" — is it 5+ min? Did you check heartbeat? |
| "I can fix it faster myself" | Maybe, but Level 3 hint takes 30 seconds. Try it first. |
| "This is critical path, can't wait" | Critical doesn't mean skip levels. Ladder exists for critical paths too. |
| "Worker is probably stuck" | "Probably" is not evidence. Read PTY (Level 1) first. |
| "I already know what the problem is" | Verify. Leaders are wrong about worker state 40% of the time. |
| "Just this once, jump to Level 4" | Level 4 is RARE. If you're reaching for it, you skipped a level. |
| "Multiple nudges failed, time to hotfix" | Were nudges actually received? Check inbox delivery. Resend if needed. |
| "Worker seems confused" | "Seems" is not evidence. Verify via PTY read and direct question. |
...
#### [V2] Level 4 Evidence Gate — MANDATORY BEFORE PROCEEDING

```
BEFORE executing ANY Level 4 action:

1. CONFIRM: Level 3 was attempted AND failed (with evidence)
2. CONFIRM: Worker received your hints (check inbox delivery status)
3. CONFIRM: This is genuinely critical path (what tasks does it block?)
4. CONFIRM: Your fix is correct (verify the problem first, don't guess)
5. DOCUMENT: Write the rationale BEFORE making the fix
6. ONLY THEN: Execute the hotfix

If you cannot confirm ALL 5 prerequisites, DROP BACK to Level 3.
```

**Red Flag Words at Level 4:**
If you're thinking any of these, you haven't done enough verification:
- "should fix it"
- "probably the issue"
- "I think the problem is"
- "this might work"

Level 4 requires CERTAINTY, not guesses.
